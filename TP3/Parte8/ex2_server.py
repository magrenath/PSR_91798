    ...
    try:  # show who connected to us
        ...
        while True:  # receive the data in small chunks (64 bytes) and print it
            ...
            if data:
                ...
                # Deserialize or unmarshall - done after we recieve data
                messages = data.split(',')

                print(messages)
                dog = ex2_dogLib.Dog(name = messages[0], color = messages[3], age = messages[2], owner = messages[1])
                for brothers in range(4, len(messages)):
                    dog.addBrother(messages[brothers])
